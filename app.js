...
/* (3)  Crea una ruta GET que: 
  ...
  NOTA: Usé el parametro dog en lugar de animals porque el ejs usa ese.
*/ 
app.get('/dog', (req, res) => {
...
app.get('/api/getAnimal/:animal', (req, res) => {
  var searched = req.params.animal;
  var index = animals.length;
  for (let i = 0; i < animals.length; i++) {
    if( animals[i].animalType === searched){
      index = i;
    }
  }
  return res.render('pages/any', {title: searched, animal: animals[index]});
});
